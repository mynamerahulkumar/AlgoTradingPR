	...
	def get_live_pnl(self):
		...
		try:
			...
			for pos_ in pos_book_dict:
				...
				Total_MTM = (float(pos_['sellAvg']) - float(pos_['buyAvg'])) + (int(pos_['netQty']) *closePrice * float(pos_['multiplier']))
		...
		try:
			...
			if scrip_type == 'CE':
				...
				if flag == "gamma":
					return cval.gamma
				if flag == "all_val":
					return {'callPrice' : round(cval.callPrice, 1), 'callDelta' : round(cval.callDelta, 1), 'callDelta2' : round(cval.callDelta2, 1), 'callTheta' : round(cval.callTheta, 1), 'callRho' : round(cval.callRho, 1), 'vega' : round(cval.vega, 1), 'gamma' : round(cval.gamma, 4)}
